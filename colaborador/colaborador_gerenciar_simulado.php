...
if (isset($_POST['deletar'])) {
  ...
  // Buscar PDF para deletar
  $sim = mysqli_fetch_assoc(mysqli_query($conn, "SELECT arquivo_pdf FROM simulados WHERE id = $simulado_id"));
  if ($sim && !empty($sim['arquivo_pdf']) && file_exists("simulados_pdf/" . $sim['arquivo_pdf'])) {
    unlink("simulados_pdf/" . $sim['arquivo_pdf']);
  }
...
function construirUrl($params) {
  ...
  return '_gerenciar_simulado.php?' . http_build_query(array_merge($filtros, $params));
}
?>
...
<head>
  ...
  <title>Gerenciar Simulados - Colaborador</title>
  ...
  <style>
    ...
    .simulado-card:hover {
      ...
      box-shadow: 0 8px 20px rgba(132, 218, 115, 0.73);
    }
...
<div class="container mt-4 mb-5">
  ...
  <div class="top-actions">
    ...
    <a href="colaborador_criar_simulado.php" class="btn-novo">
      ➕ Criar Novo Simulado
    </a>
  </div>
  ...
  <div class="filtro-bar">
    ...
    <form method="GET">
      ...
      <div class="d-flex gap-2 mt-3">
        <a href="colaborador_gerenciar_simulado.php" class="btn btn-secondary" style="border-radius: 10px; padding: 12px 20px;">
          🔄 Limpar Filtros
        </a>
      </div>
    </form>
    ...
    <?php while($sim = mysqli_fetch_assoc($res_simulados)): ?>
      <div class="simulado-card <?= $sim['ativo'] ? '' : 'inativo' ?>">
        ...
        <div class="actions-container">
          <a href="simulados_main_colaborador.php?visualizar=<?= $sim['id'] ?>" 
             class="btn-action btn-visualizar" target="_blank">
            👁️ Visualizar
          </a>
          
          <?php if (!empty($sim['arquivo_pdf']) && file_exists("simulados_pdf/" . $sim['arquivo_pdf'])): ?>
            <a href="simulados_pdf/<?= htmlspecialchars($sim['arquivo_pdf']) ?>" 
               class="btn-action btn-pdf" target="_blank">
              📄 Baixar PDF
            </a>
          <?php endif; ?>
          
          <a href="colaboador_editar_simulado.php?id=<?= $sim['id'] ?>" 
  ...
  <?php else: ?>
    <div class="empty-state">
      ...
      <p>Tente ajustar os filtros ou <a href="colaborador_criar_simulado.php">crie um novo simulado</a></p>
    </div>
  <?php endif; ?>
